        ...
        steps: [
            {
                title: "1次方程式・不等式",
                explanation: `
  「1次方程式」とは、xなどの文字が1回だけ出てくる式（ax+b=0）のことです。
...
`,
            ...
            },
            {
                title: "2次方程式",
                explanation: `
  「2次方程式」とは、x^2が出てくる式（ax^2+bx+c=0）のことです。
  解き方はいくつかあります。
  ・因数分解（かっこにまとめる）
  ・平方完成（x^2の形に直す）
  ・解の公式（どんな式でも使える便利な公式）

  また、「判別式D=b^2-4ac」を使うと、答えが何個あるかも分かります。
...
`,
            ...
            },
            {
                title: "2次不等式",
                explanation: `
  「2次不等式」とは、x^2が出てくる不等式（ax^2+bx+c>0など）のことです。
...
`,
  ...
  aがプラスなら下に開いたU字型、マイナスなら上に開いた山型になります。
...
`,
            ...
            },
            {
                title: "2次関数の最大・最小",
                explanation: `
  2次関数の「最大値・最小値」は、グラフの頂点や、xの範囲（定義域）の端で決まります。
  まず頂点のy座標を調べ、次にxの範囲の端でもyの値を計算して、一番大きい・小さい値を比べます。
...
`,
  ...
  不等式の証明では、(A-B)が0以上になることを示したり、平均の性質を使ったりします。
...
`,
            ...
            },
            {
                title: "因数定理と高次方程式",
                explanation: `
  「因数定理」は、多項式P(x)が(x-k)で割り切れるとき、P(k)=0になるという定理です。
  これを使うと、3次や4次など高い次数の方程式の解を見つけやすくなります。

  P(k)=0になるkを見つけたら、(x-k)で割って次数を下げていきます。
...
`,
            ...
            },
            {
                title: "円の方程式",
                explanation: `
  「円の方程式」は、中心と半径が分かれば(x-a)^2+(y-b)^2=r^2の形で表せます。
  これは「中心からの距離が常に半径r」という意味です。
        ...
        steps: [
            {
                title: "三角関数の定義とグラフ",
                explanation: `
  「三角関数」は、単位円（半径1の円）を使って、角度とx・yの関係を表す関数です。
  sinθはy座標、cosθはx座標、tanθは傾きとして考えられます。
...
`,
            ...
            },
            {
                title: "加法定理",
                explanation: `
  「加法定理」は、2つの角度の和や差の三角関数を、sinやcosで表す公式です。
  たとえば、sin(α+β)やcos(α+β)など。
        ...
        steps: [
            {
                ...
                explanation: `
  「指数関数」は、y=a^xの形の関数です。
  aが1より大きいと右上がり、0より小さいと右下がりのグラフになります。

  計算には「指数法則」（a^m×a^n=a^{m+n}など）を使います。
...
`,
            ...
            },
            {
                title: "対数関数",
                explanation: `
  「対数関数」は、指数関数の逆で「aを何回かけたらxになるか」を表す関数です。
  たとえば、log_2 8=3は「2を3回かけると8になる」という意味です。
...
`,
  ...
  x^nの微分は「nを前に出して、xの指数を1つ下げる」ルールで計算します。
...
`,
  ...
  x^nの積分は「指数を1つ増やして、その数で割る」ルールで計算します。
...
`,
            ...
            },
            {
                title: "定積分と面積 (多項式)",
                explanation: `
  「定積分」は、関数とx軸で囲まれた部分の「面積」を求める計算です。
  上端と下端の値を代入して引き算します。
        ...
        steps: [
            {
                title: "複素数と複素数平面",
                explanation: `
  「複素数」とは、a+biの形で表す数で、aは実数、biは虚数です。
